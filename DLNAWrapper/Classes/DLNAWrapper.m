...
- (void)startAndSearch:(BOOL)isSearch
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(onNetworkStatusChange:)
    ...
    if (isSearch)
    {
        ...
        {
            // When the current network is wifi, the device is actively searched, and non-wifi does not actively search.
            [self search];
        }
    }
}
...
- (void)search
{
    ...
    [_udpSocket sendData:[SEARCH_DATA dataUsingEncoding:NSUTF8StringEncoding]
                  toHost:UDP_SERVER_HOST
                    port:UDP_SERVER_PROT
             withTimeout:-1
                     tag:1];
}
